...
nrow(contest_basic1)
####分为测试集和训练集###
contest_basic1_train<-left_join(contest_basic_train[,c(1,11)],contest_basic1)
nrow(contest_basic1_train)
contest_basic1_test<-left_join(contest_basic_test[,1],contest_basic1)
nrow(contest_basic1_test)
...
##与基础表合并
union1<-left_join(contest_basic1_train[,-c(3,4,12)],contest_ext_crd_hd_report[,-2])
nrow(union1)
...
union2_1<-left_join(union1[,c(1,2)],daikuanbiao)[,-2]
apply(union2_1,2,function(x) sum(is.na(x)))
#合并部分缺失理解为用户没有贷款，故合并产生的缺失用0补齐
union2_1[is.na(union2_1)]<-0
union2<-left_join(union1,union2_1)
...
union3_1<-left_join(union2[,c(1,2)],daijikabiao)[,-2]
apply(union3_1,2,function(x) sum(is.na(x)))
#合并部分缺失理解为用户没有贷记卡，故合并产生的缺失用0补齐
union3_1[is.na(union3_1)]<-0
union3<-left_join(union2,union3_1)
...
nrow(union4)
union4[is.na(union4$STANDARD_LOANCARD_COUNT),]#缺失部分
...
str(sharebiao)
####与上面特征合并
union5_1<-left_join(union4[,c(1,2)],sharebiao)[,-2]
#合并部分缺失理解为用户没有贷记卡，故合并产生的缺失用0补齐
union5_1[is.na(union5_1)]<-0
union5<-left_join(union4,union5_1)
union5<-left_join(union4,sharebiao)

table(apply(union5[,-c(1:10)],1,function(x) sum(is.na(x))))
str(union5)
...
##与上面的特征合并
union6_1<-left_join(union5[,c(1,2)],ovdsummary)[,-2]
#合并部分缺失理解为用户没有贷记卡，故合并产生的缺失用0补齐
union6_1[is.na(union6_1)]<-0
union6<-left_join(union5,union6_1)
str(union6)
...
union7_1<-left_join(union6[,c(1,2)],info)[,-2]
#合并部分缺失理解为用户没有贷记卡，故合并产生的缺失用0补齐
union7_1[is.na(union7_1)]<-0
union7<-left_join(union6,union7_1)
str(union7)
apply(union7,2,function(x) sum(is.na(x)))
union7<-left_join(union5[,-c(2:10)],info)
table(apply(union7,1,function(x) sum(is.na(x))))
...
union8_1<-left_join(union7[,c(1,2)],splf)[,-2]
#合并部分缺失理解为用户没有贷记卡，故合并产生的缺失用0补齐
union8_1[is.na(union8_1)]<-0
union8<-left_join(union7,union8_1)
str(union8)
apply(union8,2,function(x) sum(is.na(x)))
union8<-left_join(union5,splf)
table(apply(union8,1,function(x) sum(is.na(x))))
...
##与上面特征合并
union9_1<-left_join(union8[,c(1,2)],lnd_ovd_mean)[,-2]
#合并部分缺失理解为用户没有贷记卡，故合并产生的缺失用0补齐
union9_1[is.na(union9_1)]<-0
union9<-left_join(union8,union9_1)
apply(union9,2,function(x) sum(is.na(x)))
str(union9)
a<-apply(union9,1,function(x) sum(is.na(x)))
b<-cbind(REPORT_ID=union9$REPORT_ID,a=a)
b<-as.data.frame(b)
b$a<-as.numeric(b$a)
max(b$a)
plot(b$a,names.arg="")
table(b$a)
###查找贷款，贷记卡，信用提示表，sharedebt表
a<-c(1,1,22);b<-c(1,1,2)
unique(a)
union(a,b)
###################用union9跑一下模型#####################
setwd("D:\\东证期货杯\\选题2竞赛数据-20171114(1)\\竞赛数据-20171114")
...
set.seed(1234)
model_rf <- train(Y ~., data = traindata2, method = 'rf', 
                  trControl = trainControl(method = 'cv', 
                                           number = 5, 
                                           selectionFunction = 'oneSE'))
model_rf
pred_rf <- predict(model_rf, testdata2[,-14])              # 进行预测
# 建立混淆矩阵
# data:预测值，reference：真实值，position：正例
testdata2$Y<-as.factor(testdata2$Y)
