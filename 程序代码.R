...
##logist模型
names(traindata2)
logit.model = glm(Y~.,data = traindata2[,-1],family = "binomial")
...
#预测集
test<-inner_join(contest_basic_test[,c(1,2)],union9)[,-2]
test_onehot<-dummy.data.frame(test,names=c("AGENT","QUERY_REASON","QUERY_ORG","IS_LOCAL","WORK_PROVINCE","EDU_LEVEL","MARRY_STATUS","HAS_FUND","sex","a","b","c"),sep=".")
standard <- preProcess(test_onehot, method = 'range')
test1 <- predict(standard, test_onehot)
names(test1)
test_pred1 <- predict(xgb, data.matrix(test1[,-1]))
test_pred2 <- predict(xgb1, data.matrix(test1[,-1]))
test_pred3 <- predict(logit.model,test[,-1],type = "response")
ph<-0.5*y_pred+0.5*y_pred1
test_pre = ifelse(test_pred1>0.5,1,0)
